import { Brain, Target, Lightbulb, CheckCircle, Loader2 } from "lucide-react";
...
interface StreamingAIAnalysisDisplayProps {
...
export default function StreamingAIAnalysisDisplay({ 
  analysis, 
  hasMatches, 
  searchQuery, 
  isStreaming,
  streamingContent 
}: StreamingAIAnalysisDisplayProps) {
  const [displayedContent, setDisplayedContent] = useState('');

  useEffect(() => {
    if (isStreaming && streamingContent) {
      setDisplayedContent(streamingContent);
    } else if (!isStreaming && analysis?.summary) {
      setDisplayedContent(analysis.summary);
    }
  }, [isStreaming, streamingContent, analysis?.summary]);
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <div className="text-blue-700 dark:text-blue-300">
              {analysis?.query_understanding || (isStreaming ? '正在理解查询...' : '')}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <div className="text-blue-700 dark:text-blue-300">
              {analysis?.search_intent || (isStreaming ? '正在识别意图...' : '')}
            </div>
          </div>
        </div>

        {/* 分析总结 - 流式显示 */}
        {(analysis?.summary || isStreaming) && (
          <div className="mt-3 pt-3 border-t border-blue-200/50 dark:border-blue-800/50">
            ...
            <div className="text-blue-700 dark:text-blue-300">
              {displayedContent || (isStreaming ? '正在生成分析...' : '')}
              {isStreaming && <span className="inline-block w-2 h-4 bg-blue-600 dark:bg-blue-400 animate-pulse ml-1" />}
            </div>
          </div>
        )}
        ...
        <div className="mt-3 pt-3 border-t border-blue-200/50 dark:border-blue-800/50">
          <div className="flex items-center justify-between text-xs text-blue-600 dark:text-blue-400">
            <span>分析了 {analysis?.total_analyzed || (isStreaming ? '...' : 0)} 个工具</span>
            <span>
              {hasMatches 
