...
WHERE status IN ('approved', 'active') 
  AND main_category IS NOT NULL 
ORDER BY main_category, sub_category, view_count DESC
LIMIT 50;
```

## ⚠️ 重要提醒

1. **移除LIMIT语法**：PostgreSQL的`array_agg`不支持`LIMIT`，已修复
2. **选择"No limit"**：在Supabase中执行时选择"No limit"选项
3. **分步执行**：建议分步执行，先执行分类分配，再查看结果
