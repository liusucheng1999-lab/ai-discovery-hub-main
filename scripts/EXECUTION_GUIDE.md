...
```
main_category | sub_category        | tool_count | sample_tools
-------------+---------------------+------------+--------------------
chat         | chat_general        | 15         | {ChatGPT,Claude,Gemini}
chat         | chat_multimodal     | 8          | {GPT-4V,Claude-3,etc}
writing      | writing_marketing   | 12         | {文案助手,SEO工具,etc}
...
```

## ⚠️ 注意事项

1. **执行时间**：根据工具数量，可能需要几秒到几十秒
2. **备份数据**：建议先备份tools表
3. **检查结果**：确保所有工具都被正确分类
4. **手动调整**：如有分类错误的工具，可以手动更新
